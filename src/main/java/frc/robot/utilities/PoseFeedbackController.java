...
public class PoseFeedbackController {
    ...
    public PoseFeedbackController(FeedbackController forward, FeedbackController sideways, FeedbackController headingDegrees) {
    ...
    public ChassisSpeeds calculate(Pose2d current, Pose2d target) {
        ...
        double rotationalSpeed = this.heading.calculate(current.getRotation().getDegrees(), target.getRotation().getDegrees());
        ...
        return new ChassisSpeeds(
            forwardSpeed,
            sidewaysSpeed,
            Math.toRadians(rotationalSpeed)
        );
    }
